...
export default function ProfileSidebar({ profile, activeSection, onSectionChange }) {
  const initials    = getInitials(profile.fullName);
  const carbonSaved = profile.stats?.carbonSaved ?? '—';
  const tripsCount  = profile.stats?.tripsTaken  ?? '—';
  const ecoScore    = profile.ecoScore ?? '—';

  return (
    <aside className="p-left">
      <div className="card border">

        {/* Avatar + identity — initials only, no upload affordance */}
        <div className="d-flex flex-column align-items-center text-center py-4 px-3">
          <div className="ps-avatar-circle mb-3">
            <span className="ps-initials">{initials}</span>
          </div>
          <div className="fw-bold mb-1" style={{ fontSize: '1rem', color: 'var(--gray-900)' }}>
            {profile.fullName}
          </div>
        ...
        <div className="d-flex justify-content-around py-3 border-top border-bottom">
          <div className="text-center">
            <div className="fw-bold mb-0" style={{ fontSize: '0.875rem', color: 'var(--gray-900)' }}>
              {carbonSaved}
            </div>
            <div style={{ fontSize: '0.65rem', color: 'var(--gray-500)', marginTop: '0.1rem', whiteSpace: 'nowrap' }}>
              CO₂ Saved
            </div>
          </div>
